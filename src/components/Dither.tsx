...
type WaveColor = [number, number, number];
...
const RetroEffect = forwardRef<Effect, Pick<DitheredWavesProps, "colorNum" | "pixelSize">>(
...
function DitheredWaves({
  ...
  const prevColor = useRef<WaveColor>([...waveColor]);
  ...
  useFrame(({ clock }) => {
    ...
    if (uniforms.waveSpeed.value !== waveSpeed) uniforms.waveSpeed.value = waveSpeed;
    if (uniforms.waveFrequency.value !== waveFrequency) {
      uniforms.waveFrequency.value = waveFrequency;
    }
    if (uniforms.waveAmplitude.value !== waveAmplitude) {
